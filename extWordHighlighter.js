...
function updateContextMenu(inUrl){
    ...
    if(inUrl&&noContextMenu.indexOf(inUrl)==-1){
        ...
        var filteredGroups=getGroups(inUrl);
...
function getGroups(inUrl){
    ...
    for (var highlightData in HighlightsData.Groups) {
        ...
        if (HighlightsData.Groups[highlightData].Enabled){
            ...
            if(returnHighlight){groupsForUrl[highlightData]=HighlightsData.Groups[highlightData];}
        }
    }
    return groupsForUrl;
}
...
function transformWordsToRegex(input){
        ...
        return {matchRegex: matchRegex, matchRegexEditable: matchRegexEditable}
}
...
function addGroup(inGroup, color, fcolor, findwords, showon, dontshowon, inWords,groupType, remoteConfig, regex, showInEditableFields) {
    for(word in inWords){
        inWords[word]=DOMPurify.sanitize(inWords[word]).replace(/(\r\n|\n|\r)/gm,"");
    }
    ...
    localStorage['HighlightsData']=JSON.stringify(HighlightsData);
    setWords(inWords, inGroup, color, fcolor, findwords, showon, dontshowon, inGroup,groupType, remoteConfig, regex, showInEditableFields);
    requestReHighlight();
    return true;
}
